...
def get_term_reduce_factor(img: Image):
    """ Input:
            img: PIL.Image - the image we want to fit onto our terminal window
        Output:
            reduce_factor: 
    """
    term_width, term_height = os.get_terminal_size()
    img_width, img_height = img.size
    height_ratio = (img_height/term_height)/2
    width_ratio = img_width/term_width
    reduce_factor = int(max(height_ratio, width_ratio))
